...
1. 引用计数（ReferenceCount）
   ...
   统计对象被引用的次数，若为0则是垃圾，但是无法清除 循环引用

2. 根可达算法(RootSearching)

   >- 线程栈变量：main线程启动，分配main线程栈，分配main栈桢存储局部变量和一些过程和结果、这些便是根。（Java 虚拟机栈所使用的内存不需要保证是连续的）
   >- 静态变量：class文件加载到内存后，经过验证，静态成员变量赋默认值，这些静态成员便是根
   >- 常量池：一个class中需要用到其他class的一些对象， 这是根
   >- JNI指针：调用本地C、C++的对象
...
- CMS比串行或并行垃圾收集器有更短的中断时间
- 牺牲一些应用的吞吐量来消除或极大地减少漫长的GC ，中断时间也是能够接受的

#### CMS 四个阶段

- 初始标记阶段 （STW）根可达算法
- 并发标记   产生垃圾+标记垃圾 （牺牲了吞吐量的阶段）采用三色标记+Incremental Update算法
- 重新标记 （STW）
- 并发清理  清理阶段会额外产生浮动垃圾

CMS的缺点：

- 在内存到好多G时，产生内存碎片，年轻代对象找不到空间时，采用Serial Old进行单线程标记压缩
